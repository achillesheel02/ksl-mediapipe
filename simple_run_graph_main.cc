  ...
  while (poller.Next(&packet)) {
    ...
    for (const ::mediapipe::NormalizedLandmarkList& landmark : output_landmarks){
        ...
        for (int i = 0; i<landmark.landmark_size(); ++i){
            const mediapipe::NormalizedLandmark& landmarkitem = landmark.landmark(i);
            if (hand_id < 1){
            absl::StrAppend(&output_data, i, "-(");
            }
            else {
            absl::StrAppend(&output_data, 21+i, "-(");
            }
            absl::StrAppend(&output_data, landmarkitem.x(), ",");
            absl::StrAppend(&output_data, landmarkitem.y(), ") ");
            }
        hand_id++;
